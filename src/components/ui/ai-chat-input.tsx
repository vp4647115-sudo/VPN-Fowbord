...
function ModelIcon({ model, className }: { model: string; className?: string }) {
  ...
  return (
    <img 
      src={icons[model] || icons["Gemini 2.5 Flash"]} 
      alt={model} 
...
export const PromptInput = React.forwardRef<HTMLDivElement, PromptInputProps>(
  (
    {
      onSubmit,
      placeholder = "Ask AI to generate architecture, diagrams, flowcharts...",
      className,
      models = ["Gemini 3.5 Flash", "Gemini 3.6 Flash", "GPT 5.5", "Opus 4.8"],
    ...
    const handleFilesChosen = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const accepted: File[] = imageFiles.slice(0, room);
        ...
        >
          ...
          >
            <div
              ...
              className="border border-slate-200 border-b-0 bg-slate-100/90 rounded-t-2xl px-2 pt-2 pb-1 flex items-start gap-2 overflow-x-auto prompt-scrollbar"
            >
          ...
          >
            <style dangerouslySetInnerHTML={{ __html: `
              ...
              .prompt-scrollbar:hover::-webkit-scrollbar-thumb { background: #cbd5e1; }
            `}} />
            ...
            <button
              ...
              className={cn(
                "absolute inset-x-0 top-0 z-[1] cursor-text pl-4 pr-12 py-[13px] text-left text-xs font-semibold leading-[18px] text-slate-600 flex items-center gap-2 outline-none",
                !expanded ? "opacity-100 scale-100 translate-y-0" : "opacity-0 scale-105 translate-y-1 pointer-events-none"
              )}
              aria-label="Open AI Prompt Input"
            >
              <span className="material-symbols-outlined text-base text-[#004ac6] animate-pulse">auto_awesome</span>
              <span className="truncate">{loading ? "AI Generating Flowchart..." : placeholder}</span>
            </button>
            ...
            >
              <div className="relative">
                <button
                  ...
                  className={cn(
                    "group flex items-center gap-1 rounded-full px-2 py-1 text-slate-600 hover:bg-slate-100 transition-all duration-200 outline-none cursor-default",
                    isModelSelectOpen ? "bg-slate-100 text-blue-700" : ""
                  )}
                ...
                <div
                  ...
                  className={cn(
                    "absolute bottom-full left-0 mb-2.5 z-50 w-44 rounded-2xl border border-slate-200 bg-white/95 p-1 shadow-2xl backdrop-blur-md flex flex-col gap-0.5 transition-all duration-400 cursor-default",
                    isModelSelectOpen
                ...
                >
                  <div className="relative flex flex-col gap-0.5">
                    <div style={hoverStyle} className="absolute left-0 right-0 top-0 h-8 -z-10 rounded-xl bg-blue-50 pointer-events-none" />
                    {models.map((model, idx) => (
                      <button
                        ...
                        className="group relative flex h-8 w-full items-center justify-between rounded-xl px-2.5 py-1.5 text-left text-xs font-medium text-slate-700 outline-none active:scale-[0.98] cursor-default"
                      >
              ...
              <button
                ...
                className="group flex items-center gap-1 rounded-full px-2 py-1 text-slate-500 transition-all duration-200 hover:bg-slate-100 hover:text-slate-800 outline-none cursor-default"
              >
            ...
            >
              {audioData.map((val, i) => (
                <div
                  key={i}
                  className="w-1 rounded-full bg-[#004ac6] transition-[height] duration-75 ease-out"
            ...
            <button
              ...
              className="absolute right-2 bottom-2 z-[10] flex h-8 w-8 items-center justify-center bg-[#004ac6] hover:bg-[#2563eb] text-white transition-all duration-300 hover:scale-105 active:scale-95 shadow-md outline-none cursor-default"
            >
              {loading ? (
